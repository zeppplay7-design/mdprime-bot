<?php
/* MDPRIME PANEL V11 - BUSCADOR CLIENTES NORMALES FORZADO - RENDER + RAILWAY */
...
<style id="mdprimeNormalesSearchV11">
.normalesSearchBox{
  margin:14px 0 16px!important;
  padding:14px!important;
  border:1px solid rgba(31,182,255,.35)!important;
  background:rgba(0,0,0,.22)!important;
  border-radius:18px!important;
}
.normalesSearchGrid{
  display:grid!important;
  grid-template-columns:1.5fr auto auto auto auto!important;
  gap:8px!important;
  align-items:center!important;
}
.normalFilterBtn{
...
@media(max-width:980px){.normalesSearchGrid{grid-template-columns:1fr 1fr!important}}
@media(max-width:760px){.normalesSearchGrid{grid-template-columns:1fr!important}}
</style>
...
<script id="mdprimeNormalesSearchJsV11">
let mdFiltroNormalActual = 'todos';

function mdNormalizaTexto(v){
  return (v || '').toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
}

function mdSetFiltroNormales(filtro, btn){
  mdFiltroNormalActual = filtro || 'todos';
  document.querySelectorAll('.normalFilterBtn').forEach(function(b){ b.classList.remove('active'); });
  if(btn){ btn.classList.add('active'); }
  mdFiltrarNormales();
}

function mdFiltrarNormales(){
  const input = document.getElementById('normalSearchInput');
  const q = mdNormalizaTexto(input ? input.value : '');
  const cards = document.querySelectorAll('.normalCard');
  let visibles = 0;

  cards.forEach(function(card){
    const texto = mdNormalizaTexto(card.getAttribute('data-search') || card.innerText || '');
    const estado = (card.getAttribute('data-status') || '').toLowerCase();
    const okTexto = q === '' || texto.indexOf(q) !== -1;
    const okFiltro = mdFiltroNormalActual === 'todos' || estado === mdFiltroNormalActual;
    const mostrar = okTexto && okFiltro;
    card.style.display = mostrar ? '' : 'none';
    if(mostrar) visibles++;
  });

  const empty = document.getElementById('normalNoResults');
  if(empty){ empty.classList.toggle('show', visibles === 0); }
}

function mdLimpiarNormales(){
  const input = document.getElementById('normalSearchInput');
  if(input){ input.value = ''; }
  mdFiltroNormalActual = 'todos';
  document.querySelectorAll('.normalFilterBtn').forEach(function(b){
    b.classList.remove('active');
    if(b.getAttribute('data-normal-filter') === 'todos'){ b.classList.add('active'); }
  });
  mdFiltrarNormales();
}
</script>
